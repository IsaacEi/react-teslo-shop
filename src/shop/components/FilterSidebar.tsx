...
const FilterSidebar = () => {
  ...
  const { sizes: sizesArray, price: currentPrice, setSearchParams, searchParams } = useProductGrid();

  const currentSizes: Size[] = sizesArray ? (Array.isArray(sizesArray) ? sizesArray as Size[] : (sizesArray.split(',') as Size[])) : [];
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          {sizes.map((size) => (
            <Button
              key={size.id}
              variant={currentSizes.includes(size.id as Size) ? 'default' : 'outline'}
              size="sm"
              className="h-8"
              onClick={() => handleSizeChange(size.id as Size)}
            >
              {size.label}
            </Button>
          ))}
